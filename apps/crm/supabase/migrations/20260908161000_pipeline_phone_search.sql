-- Aplica a comparação normalizada à busca do Funil. Mantém a versão mais
-- recente da consulta, inclusive leads de cadastro público ainda sem conversa.
begin;

create or replace function crm.pipeline_lead_has_registration(p_lead_id uuid)
returns boolean
language plpgsql
stable
security invoker
...
begin
  if to_regclass('crm.lead_registrations') is null then
    return false;
  end if;

  execute 'select exists (
    select 1 from crm.lead_registrations registration where registration.lead_id = $1
  )'
  into registered
  using p_lead_id;
...
revoke all on function crm.pipeline_lead_has_registration(uuid) from public;
grant execute on function crm.pipeline_lead_has_registration(uuid) to authenticated, service_role;
...
as $$
  ...
  where l.excluded_from_pipeline_at is null
    and (
      ...
      or crm.pipeline_lead_has_registration(l.id)
    )
    ...
    and (
      ...
      or crm.phone_search_matches(l.phone_e164, p_query)
      or crm.phone_search_matches(o.title, p_query)
    )
    and (
      p_volume is null
