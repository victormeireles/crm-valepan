-- Cadastros do evento aparecem no funil mesmo antes da primeira conversa.
-- Preserva filtros, RLS, contagens e paginação existentes; não inventa mensagens.
begin;
...
as $$
  ...
  where l.excluded_from_pipeline_at is null
    and (last_message.conversation_id is not null or exists (
      select 1 from crm.lead_registrations registration where registration.lead_id = l.id
    ))
