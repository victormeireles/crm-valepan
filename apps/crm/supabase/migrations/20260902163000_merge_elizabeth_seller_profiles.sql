...
begin
  ...
  from crm.profiles
  where lower(trim(full_name)) = 'elizabeth'
  order by created_at
  limit 1;
  ...
  from crm.profiles
  where lower(trim(full_name)) = 'elizabeth d''ávila'
  order by created_at
  limit 1;

  if source_profile_id is null or target_profile_id is null then
    raise exception 'Perfis de origem e destino da Elizabeth não foram encontrados';
  end if;

  if source_profile_id = target_profile_id then
    raise exception 'Os perfis de origem e destino da Elizabeth são iguais';
  end if;
  ...
  update crm.profiles
  set full_name = 'Elizabeth D''Ávila', updated_at = now()
  where id = target_profile_id;

  update auth.users
  set raw_user_meta_data = coalesce(raw_user_meta_data, '{}'::jsonb)
      || jsonb_build_object('full_name', 'Elizabeth D''Ávila'),
      updated_at = now()
  where id = target_profile_id;
