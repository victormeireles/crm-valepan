...
as $$
  select
    ...
    l.distributor_id, l.network_type, c.full_name, company.name, distributor.name,
  ...
  where l.excluded_from_pipeline_at is null
    ...
    and (
      ...
      or concat_ws(' ', c.full_name, company.name, o.title) ilike '%' || trim(p_query) || '%'
      or (
